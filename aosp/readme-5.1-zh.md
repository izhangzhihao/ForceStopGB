...
- [api-22.smali.patch](api-22.smali.patch), [api-22.patch](api-22.patch)
...
Linux/Mac OS X命令行> patch -p0 < api-22.smali.patch
```
...
Windows命令行> type api-22.smali.patch | more /p > api-22.win32.smali.patch
Windows命令行> p@tch.exe -p0 < api-22.win32.smali.patch
```
...
这个方式适合从头开始编译的ROM系统，只需要在 `frameworks/base` 下应用 `api-22.patch` 补丁即可。

```
命令行> cd frameworks/base
命令行> patch -p1 < /path/to/api-22.patch
